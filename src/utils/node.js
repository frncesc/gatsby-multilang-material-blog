 ...
 * @param {object} data - The GraphQL query results. Should contain the `allMarkdownRemark` component.
...
export const getResolvedVersionForLanguage = (data, intl) => {
  ...
  const { node } =
    data.allMarkdownRemark.edges.find(({ node: { fields: { lang } } }) => lang === locale) ||
    data.allMarkdownRemark.edges.find(({ node: { fields: { lang } } }) => lang === defaultLocale) ||
    emptyNode;
...
export const getAllResolvedVersionsForLanguage = (data, intl) => {
  ...
  slugs.forEach(sl => {
    const { node } =
      data.allMarkdownRemark.edges.find(({ node: { fields: { lang, slug } } }) => slug === sl && lang === locale) ||
      data.allMarkdownRemark.edges.find(({ node: { fields: { lang, slug } } }) => slug === sl && lang === defaultLocale);
    if (node)
      nodes.push(node);
  });
